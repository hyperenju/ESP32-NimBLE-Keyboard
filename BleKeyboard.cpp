...
extern const uint8_t _jasciimap[128] PROGMEM;
...
// JIS版ASCII HID USAGE ID 変換を使ったpress
...
// JIS版ASCII HID USAGE ID 変換を使ったrelease
size_t BleKeyboard::jrelease(uint8_t k)
{
	uint8_t i;
	if (k >= 136)
	{ // it's a non-printing key (not a modifier)
